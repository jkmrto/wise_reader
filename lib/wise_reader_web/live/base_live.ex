defmodule WiseReaderWeb.BaseLive do
  ...
  def handle_event("import-bankinter", _value, socket) do
    output =
      consume_uploaded_entries(socket, :import_bankinter, fn %{path: path} = args,
        ...
        |> Enum.map(fn row_string ->
          [date_str, description, "Crédito", amount, _] = String.split(row_string, ";")
          ...
          |> Enum.each(& WiseReader.Repo.insert(&1))

        {:ok, filename}
      end)

    socket = put_flash(socket, :info, "The file #{output} was correctly imported")
  ...
  # TODO: move to context modules (Maybe bankinter parser)
